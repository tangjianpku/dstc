...
class Model(object):
  ...
  def get_model_dir(self):
    ...
    #for attr in self._attrs:
     ...
     # if hasattr(self, attr):
      #  model_dir += "_%s_%s" % (attr, getattr(self, attr))
    return model_dir
  ...
  def initialize(self, log_dir="./logs"):
    ...
    self.writer = tf.train.SummaryWriter(log_dir, self._sess.graph_def)
  ...
  def load(self, checkpoint_dir):
    ...
    if ckpt and ckpt.model_checkpoint_path:
      ...
      self.saver.restore(self._sess, os.path.join(checkpoint_dir, ckpt_name))
